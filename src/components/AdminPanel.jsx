...
const AdminPanel = ({ user, onLogout }) => {
  ...
  const handleFaceUpload = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:5000/api/admin/students/${currentStudentForFace._id}/face`, formData);
      setSuccess(response.data.message);
      setOpenFaceUploadModal(false);
    } catch (err) {
      setError(err.response?.data?.error || "Yüz verisi yüklenirken bir hata oluştu.");
    } finally {
      setLoading(false);
    }
  };
